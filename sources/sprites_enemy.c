...
/*   Updated: 2025/11/05 18:57:19 by migusant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	load_enemy_base_paths(t_game *game, char **base_paths)
{
	...
	while (++anim < ENEMY_ANIMATIONS)
	{
		...
		while (++frame < ANIMATION_FRAMES)
		{
			...
			free(frame_str);
			game->enemies.sprites[anim][frame] = mlx_xpm_file_to_image(
					game->mlx, path, &game->img_width, &game->img_height);
			if (!game->enemies.sprites[anim][frame])
...
static int	load_enemy_static_paths(t_game *game, char **static_paths)
{
	...
	{
		game->enemies.static_sprites[anim] = mlx_xpm_file_to_image(game->mlx,
				static_paths[anim], &game->img_width, &game->img_height);
		if (!game->enemies.static_sprites[anim])
			return (ft_printf("Error\nCould not load: %s\n",
					static_paths[anim]), 0);
		anim++;
	}
